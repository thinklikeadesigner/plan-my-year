# Q1 2026 (February - March) — Phases 1-3

## Phase 1: Finish the Weapon (Feb 8-28)

### DSA — Structy is primary (10-14 hrs/week from Core + Flex)
- [ ] **Finish Structy** (11/114 done, 103 remaining — ~25-30/week across 3 weeks + overflow)
  - HIGH priority: DP (15 left), Graph II (16 left), Exhaustive Recursion (12 left)
  - Then: Stack (7), Array+String (10), Linked List II (5), Binary Tree II (12), Heap (7)
  - Last: Mixed Recall (19) — overflow into Mar 1-7
- [ ] 30-day Instagram challenge continues independently (1 problem/day)
- [ ] NeetCode 150: deferred to after Structy — becomes primary in Phase 2+
- [ ] Explain approach out loud. No IDE autocomplete.
- [ ] Can't solve in time limit → study solution 10 min, re-attempt in 3 days

### DDIA (2 hrs/week)
- [ ] Chapter 5: Replication (Week of Feb 9)
- [ ] Chapter 6: Partitioning (Week of Feb 16)
- [ ] Chapter 7: Transactions (Week of Feb 23)
- [ ] Take notes as if teaching each chapter. 1-paragraph summary after each.

### Networking (2 hrs/week)
- [ ] Follow up with Amazon engineer (coffee or run, late Feb)
- [ ] Attend Founders Run Club at least once before hackathon
- [ ] Attend one ATX Divas Run Club
- [ ] **OpenClaw hackathon Feb 13-14** — networking > winning. Follow up contacts within 48 hrs.
- [ ] **YC founders hackathon Feb 20** (tentative, 5 hrs) — go if strong networking potential
- [ ] **UT AI/Memory hackathon Feb 25** (6-8:30pm) — different crowd, low cost
- [ ] After each hackathon: back to Structy next day. No post-hackathon iteration.

### LinkedIn (1 hr/week)
- [ ] Post AI Refactor carousel (Week 1 of content)
- [ ] Goes up BEFORE any applications

---

## Phase 2: System Design Sprint (Mar 1-21)

### System Design (5 hrs/week)
- [ ] One Educative/Grokking problem per day, 6 days/week = 18 designs
- [ ] Sketch own design in 20 min BEFORE looking at answer
- [ ] Priority order (platform/infra focus):
  1. Rate limiter
  2. Distributed cache
  3. Message queue / pub-sub
  4. Web crawler
  5. **Search indexing system** (home turf — "I actually built one")
  6. Notification service
  7. URL shortener
  8. Key-value store
  9. Distributed file storage
  10. Monitoring/alerting system

**Secret weapon:** Weave in real stories. Indexing pipeline for search design. Workday sync for data pipeline design. Hash-based dedup for image processing design.

### DSA Maintenance: Targeted NeetCode Hard (30-60 min/day)
**Goal:** Keep DSA sharp, practice harder problems, target weak areas

**Week 1 (Mar 1-7): Finish Mixed Recall + Graph Hard**
- [ ] Finish Structy Mixed Recall (overflow from Feb)
- [ ] 3-5 NeetCode Hard Graph problems (your weakest area from Structy)

**Week 2-3 (Mar 8-21): DP + Backtracking Hard**
- [ ] 3-5 NeetCode Hard DP problems/week (52 min avg in Structy—sharpen this)
- [ ] 3-5 NeetCode Hard Backtracking problems/week (Exhaustive Recursion weak area)

**Total March NeetCode:** 20-25 Hard problems (reinforcement, not new learning)

**Why Hard problems:** Google trending toward LC Hard in 2025. Practice explaining complex approaches out loud.

### DDIA (2 hrs/week)
**Audiobook on commutes (Tue/Wed office days):**
- [ ] Ch 3: Storage & Retrieval (if not done in Feb)
- [ ] Ch 4: Encoding & Evolution (Week of Mar 2)

**Intentional reading:**
- [ ] Chapter 8: The Trouble with Distributed Systems (Week of Mar 2-9)
- [ ] Chapter 9: Consistency and Consensus (Week of Mar 9-16)
- [ ] Review + synthesize chapters 5-9 (Week of Mar 16-21)

### Networking (2 hrs/week)
- [ ] Coffee chat with Isha (Amazon engineer) — aim for Week of Mar 3 or Mar 10
- [ ] Follow up with hackathon contacts within 48 hours (LinkedIn + "great building with you")
- [ ] **Deliver Elastic Austin meetup talk** (likely mid-March, TBD with Sophia)
- [ ] Post Elastic talk carousel on LinkedIn (after presentation)
- [ ] Engage meaningfully on 3-5 posts/week from engineers at target companies

---

## Phase 3 Start: Behavioral + Mocks (Mar 22-31)

### Behavioral Story Bank (Week of Mar 22)
- [ ] Story 1: Indexing Pipeline (STAR format, timed at 2 min)
- [ ] Story 2: Workday Integration
- [ ] Story 3: Image Dedup Pipeline
- [ ] Story 4: IoT Form Redesign
- [ ] Story 5: AI-Assisted MVC Refactor
- [ ] Story 6: The Showing-Up Story

### Mock Interviews (start Week of Mar 29)
- [ ] Schedule 2 coding mocks on Pramp (free)
- [ ] Schedule 2 coding mocks on Interviewing.io (paid)
- [ ] Find system design mock partner (Austin network or Interviewing.io)
- [ ] Find behavioral mock partner

**Coding mocks:** Use NeetCode Hard problems (10-15 problems via mocks in April = live coding practice)

After each mock, debrief: What went well? Where did I freeze? What surprised me? What would I do differently?

### Networking
- [ ] If Isha (Amazon) coffee chat went well → referral ask: "I'm starting to look at platform/infra roles. If you think it might be a fit, I'd love to learn about the referral process."
- [ ] Olivia/Sophia (Elastic) — stay warm, potential referral path from talk collaboration
- [ ] Post brand statement synthesis on LinkedIn (after Elastic carousel is up)

---

## Key Dates

| Date | Event |
|------|-------|
| Feb 8 | Phase 1 starts — Ultra Plan activated |
| Feb 13-14 | OpenClaw AI hackathon (Fri 12pm → Sat 6pm) |
| Feb 20 | YC founders hackathon (Fri, 5 hrs — tentative) |
| Feb 25 | UT AI/Memory hackathon (Wed, 6-8:30pm) |
| Feb 28 | Phase 1 target — Structy done (except Mixed Recall overflow), DDIA Ch 5-7 |
| Mar 1-7 | Mixed Recall mop-up, transition to Phase 2 |
| Mar 1 | Phase 2 starts — System Design Sprint |
| Mar 21 | Phase 2 complete — 18 system design problems done |
| Mar 22 | Phase 3 starts — Behavioral prep |
| Mar 31 | Q1 Review — 6 STAR stories prepped, first mocks scheduled |

---

## Q1 Fitness & Weight

| Metric | Start | End of Feb | End of Mar |
|--------|-------|------------|------------|
| Weight | 156.5 lbs | ~153.5 lbs | ~150.5 lbs |
| Routine | 5x/week | Consistent | Consistent |

- Boulder/Lift: Thu, Sat
- Walk/Run: Tue, Wed, Fri
- Rest: Sun

---

## Weekly Non-Negotiables

**Phase 1 (Feb):**
- [ ] 25-30 Structy problems/week (finish 114 by Feb 28)
- [ ] 1 networking touchpoint (message, coffee, event)
- [ ] 1 LinkedIn engagement session

**Phase 2 (Mar):**
- [ ] 1 system design problem/day (6 days/week = 18 total)
- [ ] 3-5 NeetCode Hard problems/week (20-25 total in March)
- [ ] 1 networking touchpoint (message, coffee, event, Elastic talk)
- [ ] 1 LinkedIn engagement session

**Phase 3 (Late Mar/Apr):**
- [ ] 2 behavioral stories rehearsed out loud/week
- [ ] 2-3 mock interviews/week
- [ ] 3-5 NeetCode Hard via mocks (10-15 total in April)
- [ ] 1 networking touchpoint
