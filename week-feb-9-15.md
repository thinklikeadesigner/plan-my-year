# Week 2: February 9-15, 2026

**Phase 1: Finish the Weapon — Structy is the priority.**
**OpenClaw hackathon Fri-Sat. Lighter study week. Front-load Mon-Thu.**

---

## Weekly Targets

### Structy (25 problems)
- [ ] DP: finish remaining 15 (currently 7/22)
- [ ] Stack: finish all 7 (currently 0/7)
- [ ] Graph II: start, aim for 3 (currently 0/16)

### DDIA
- [ ] Chapter 5: Replication (2 hrs)

### Networking
- [ ] Founders Run Club (if event this week)
- [ ] OpenClaw hackathon: relationships > winning. Exchange contacts.
- [ ] Follow up with all OpenClaw contacts within 48 hrs (by Mon Feb 16)

### LinkedIn
- [ ] Contact Isha (Amazon) by Sun Feb 15 (moved from carousel - higher priority)
- [ ] Optional: Simple text post about Elastic talk opportunity

### Fitness
- [ ] 4 sessions (Tue Walk/Run, Wed Walk/Run, Thu Boulder/Lift, Fri Walk/Run)
- [ ] Sat Boulder/Lift: SKIP (hackathon)

### Weight
- Start: 156.1 lbs
- Target: ~155.4 lbs

---

## Daily Breakdown

| Day | Date | Fitness | Study Hrs | Focus |
|-----|------|---------|-----------|-------|
| **Mon** | Feb 9 | — | 3 (2C+1F) | Structy DP |
| **Tue** | Feb 10 | Walk/Run | 2 (2C) | Structy DP |
| **Wed** | Feb 11 | Walk/Run | 2 (2C) | Structy DP → Stack |
| **Thu** | Feb 12 | Boulder/Lift | 4 (2C+2F) | Structy Stack + DDIA Ch 5 |
| **Fri** | Feb 13 | Walk/Run (AM) | 6 (4C+2F) | **Structy + DDIA → OpenClaw hackathon (5pm →)** |
| **Sat** | Feb 14 | Skip | 0 | **OpenClaw hackathon (→ 6pm)** |
| **Sun** | Feb 15 | Rest | 4 (2C+2F) | Structy Graph II + DDIA Ch 5 + Review |

**Total: 21 hrs study (17 hrs Structy, 2 hrs DDIA, 1 hr LinkedIn, 1 hr networking)**

---

## Daily Details

### Monday, Feb 9 — WFH, Weekly Kickoff
- [x] 6:30am: Wake
- [x] 6:30-7:30: Morning routine + **Weekly Kickoff** (10 min: scan this file, commit to week)
- [x] 8:00-4:00pm: Work
- [x] 4:00-5:30: Break, dinner
- [x] 5:30-7:30: **Structy DP** (~5 problems) — CORE
- [x] 7:30-8:30: **Structy DP** (~2-3 problems) — FLEX
- [x] 11:00pm: Bed

**Day target: 7-8 DP problems. DP done count: 7 → ~14-15.**

### Tuesday, Feb 10 — Office day, Walk/Run
- [ ] 6:30am: Wake
- [ ] 6:30-7:15: Walk/Run
- [ ] 8:00am: Leave (1h45m drive)
- [ ] 10:00-6:00pm: Work
- [ ] 6:30-8:30: **Structy DP** (~4-5 problems) — CORE
- [ ] 11:00pm: Bed

**Day target: 4-5 DP problems. DP running total: ~18-20.**

### Wednesday, Feb 11 — Office day, Walk/Run
- [ ] 6:30am: Wake
- [ ] 6:30-7:15: Walk/Run
- [ ] 7:45am: Leave (15m drive)
- [ ] 8:00-4:00pm: Work
- [ ] 4:00pm: Leave (1h45m drive home)
- [ ] 6:00-8:00: **Structy DP → Stack** (finish DP, start Stack ~2-4 problems) — CORE
- [ ] Contact Isha (Amazon): casual message about coffee/run this month (10 min)
- [ ] 11:00pm: Bed

**Day target: finish DP section (2-4 remaining) + start Stack. DP: 22/22 done.**

### Thursday, Feb 12 — Gym → WFH, Boulder/Lift (Best study day)
- [ ] 5:30am: Wake
- [ ] 5:40am: Leave for gym
- [ ] 6:15-7:45am: Boulder/Lift
- [ ] 8:00am-12:00pm: Work from gym
- [ ] 12:00pm: Drive home
- [ ] 12:35-4:00pm: Work from home
- [ ] 5:00-7:00: **Structy Stack** (finish remaining ~3-5 problems) — CORE
- [ ] 7:00-9:00: **DDIA Ch 5: Replication** (1.5 hrs reading + notes) — FLEX
- [ ] 11:00pm: Bed

**Day target: Finish Stack section (7/7). Start DDIA Ch 5.**

### Friday, Feb 13 — PTO: Walk/Run → Study → OpenClaw Hackathon
- [ ] 6:30am: Wake
- [ ] 6:30-7:15: Walk/Run
- [ ] 8:00-12:00: **Structy** (~6-8 problems) — CORE
- [ ] 12:00-1:00: Lunch
- [ ] 1:00-3:00: **Structy** (~3-4 problems) — CORE
- [ ] 3:00-4:30: **DDIA Ch 5** — FLEX
- [ ] **5:00pm: OpenClaw AI Hackathon begins**
- [ ] Build something tight with tools you know (Python, APIs)
- [ ] Meet engineers. Exchange contacts. Be memorable.

### Saturday, Feb 14 — OpenClaw Hackathon → Recovery
- [ ] **OpenClaw hackathon continues → 6pm**
- [ ] Skip Boulder/Lift (at hackathon)
- [ ] 6:00pm: Hackathon ends. Ship it.
- [ ] Evening: Rest, recover. You earned it.

**Hackathon rule: Build, ship, leave, move on. No multi-day iteration.**

### Sunday, Feb 15 — Rest + Review + Catch Up
- [ ] 8:00am: Wake (sleep in, no guilt)
- [ ] 8:00-10:00: Morning routine, meal prep
- [ ] 10:00-12:00: **Structy Graph II** (start, aim for 3 problems) — CORE
- [ ] 12:00-2:00: Lunch, rest
- [ ] 2:00-3:00: **DDIA Ch 5: Replication** (finish reading, write 1-paragraph summary) — FLEX
- [ ] 3:00-4:00: **LinkedIn engagement** + follow up OpenClaw contacts — FLEX
- [ ] 4:00-5:00: **Weekly Review**
- [ ] Update tracker.csv
- [ ] Run `next week` command
- [ ] 11:00pm: Bed

**Day target: Graph II 3/16. DDIA Ch 5 complete.**

---

## Structy Progress Tracker

| Section | Start of Week | Target End of Week | Status |
|---------|---------------|-------------------|--------|
| DP | 7/22 | 22/22 | Finish this week |
| Stack | 0/7 | 7/7 | Finish this week |
| Graph II | 0/16 | 3/16 | Start this week |

**Running total after W2: 36/114 (25 new + 11 existing)**

---

## Weekly Review (Sunday)

### Progress Check
- [ ] Structy problems completed: _____ / 25
- [ ] DP section: _____ / 22
- [ ] Stack section: _____ / 7
- [ ] Graph II section: _____ / 3 (started)
- [ ] DDIA Ch 5 read: [ ] yes [ ] no
- [ ] DDIA summary written: [ ] yes [ ] no
- [ ] LinkedIn carousel posted: [ ] yes [ ] no
- [ ] OpenClaw contacts followed up: [ ] yes [ ] no
- [ ] Fitness sessions: _____ / 4
- [ ] Weight: _____ lbs

### What worked?


### What didn't?


### Energy levels?
- [ ] High
- [ ] Medium
- [ ] Low — consider reducing flex hours next week

### Adjustments for next week?


---

## Notes

- Hackathon shifted to 5pm Friday. PTO day = bonus study block. 21 hrs study instead of original 15.
- DP is the hardest section. If you're stuck on a problem for >25 min: study the solution for 10 min, move on, re-attempt in 3 days.
- Explain your approach out loud on every problem. Interview practice starts now.
